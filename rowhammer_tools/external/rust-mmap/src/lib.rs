...
impl MemoryMap {
    ...
    //RUBEN
    pub fn mremap(&mut self, new_page_addr: *mut u8) {
        let flags = libc::MREMAP_FIXED | libc::MREMAP_MAYMOVE;
        let page_size = page_size();
        unsafe {
          let t = libc::mremap(self.data() as *mut c_void, page_size,
                               page_size, flags, new_page_addr as *mut c_void);
          assert!(t as *mut u8 == new_page_addr);
        }

        self.data = new_page_addr;
    }

    //RUBEN
    //takes ownership of MemoryMap, calls drop() at the end, munmap will unmap
    //nothing and the page stays mapped
