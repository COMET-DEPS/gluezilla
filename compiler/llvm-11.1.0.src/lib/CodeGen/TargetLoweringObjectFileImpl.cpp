...
getELFSectionNameForGlobal(const GlobalObject *GO, SectionKind Kind,
  ...
  if (Kind.isMergeableCString()) {
    ...
    //DBL we put almost all functions in our ".dbl_text" section instead of
    //in the ".text" section, except for the new main function and the
    //entry function into the loader
    if (Name == ".text") {
      auto* Fn = dyn_cast<Function>(GO);
      std::regex DtorRegex(".*D[012]Ev$");
      std::regex CtorRegex(".*C([123]|I[12])Ev$");

      if (DBLMode != Baseline && Fn && Fn->getName() != "main" &&
         Fn->getName() != "do_the_thing" &&
         //the following rules are only required when the target offsets are
         //automatically generated. We only want explicit source code in the
         //.dbl_text section (ie, no std lib fns, no implicit code like
         //implicit destructors)
         !Fn->getName().slice(0, 10).contains("St") && //std::
         !Fn->getName().contains("__gnu_cxx") && //__gnu_cxx::
         //for simplicity, put all ctors and dtors in .text
         !std::regex_match(Fn->getName().str(), DtorRegex) && //dtors
         !std::regex_match(Fn->getName().str(), CtorRegex)) //ctors
        //change the name to our .dbl_text section
        Name = ".dbl_text";
    }
  }
...
static MCSectionELF *selectELFSectionForGlobal(
  ...
  if (EmitUniqueSection) {
    if (TM.getUniqueSectionNames()) {
      //DBL when using lld, unique sections are created for each function
      //eg: .text.fn instead of just putting everything in .text
      //-funique-section-names and -fno-function-sections do not seem te work?
      //TODO look for source of UniqueSectionNames
      //For now, disable it here
      UniqueSectionName = false; //true;
    } else {
      UniqueID = *NextUniqueID;
      (*NextUniqueID)++;
    }
