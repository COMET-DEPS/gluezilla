...
MCObjectStreamer::getOrCreateDataFragment(const MCSubtargetInfo *STI) {
  ...
  MCDataFragment *F;
  if(DBLMode != Baseline) {
    //create new MCDataFragments for each instruction
    //Fragments are easier to bundle than to split (bcs of fixups) => first emit
    //1 instr per fragment, later bundle them together to the right size
    F = new MCDataFragment();
    insert(F);
  } else {
    F = dyn_cast_or_null<MCDataFragment>(getCurrentFragment());
