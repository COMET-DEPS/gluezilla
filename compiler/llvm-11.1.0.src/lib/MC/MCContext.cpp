...
MCSectionELF *MCContext::getELFSection(const Twine &Section, unsigned Type,
  ...
  //DBL create at most one .dbl_text section
  MCSectionELF *Result = nullptr;
  if(Section.str() == ".dbl_text")
    Result = (MCSectionELF*)getObjectFileInfo()->getDBLTextSection();
  if(!Result)
    Result = createELFSectionImpl(CachedName, Type, Flags, Kind, EntrySize,
                                  GroupSym, UniqueID, LinkedToSym);
  Entry.second = Result;
