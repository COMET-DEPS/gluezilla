...
public:
  //DBL
  const uint64_t PageSizeBits = 12;
  const uint64_t PageSize = 1 << PageSizeBits;
  const uint64_t PageOffsetMask = (1 << PageSizeBits) - 1;
  const uint64_t PageAddrMask = ~PageOffsetMask;

  //represents a victim address from victim_addresses.txt
  //TODO put together with target structs
  struct VictimInfo {
    uint64_t VictimAddr;
    uint64_t Bit;
    bool Sign;
  ...
  //victim infos grouped per page frame
  mutable std::vector<std::vector<VictimInfo>> VictimInfos;

  //contains the targets to look for in the binary, grouped per section
  //sectionName -> [target offset]
  mutable std::map<std::string,
                   std::list<TargetSpec>> TargetsToFindForSection;

  //This represents a group of consecutive MCFragments that gets moved as
  //a whole while constructing the custom binary layout
  //(a bundle is like a single puzzle piece)
  //The division in bundles is currently done by simply cutting the fragments
  //list at fragments that contain a TargetSpec instance (and thus contain a
  //bitflip or is the destination of a TargetRange) => each bundle has a
  //TargetSpec in its first fragment
  struct Bundle {
    TargetSpec Spec;
    MCSection::iterator BundleBegin;
    MCSection::iterator BundleEnd; //inclusive!

    uint64_t getBundleSize(const MCAsmLayout &Layout, const MCAssembler& Asm) {
      uint64_t Size = 0;
      for(auto Itr = BundleBegin; Itr != std::next(BundleEnd); Itr++)
        Size += Asm.computeFragmentSize(Layout, *Itr);
      return Size;
    }

    std::string str(int I, const MCAsmLayout &Layout, const MCAssembler& Asm) {
      std::stringstream Str("");
      Str << "Bundle " << I <<" : " << std::distance(BundleBegin, BundleEnd) + 1
          << " fragments, size: " << getBundleSize(Layout, Asm) << " bytes, ";
      if(auto* P = std::get_if<TargetFixed>(&Spec.Kind)) {
        Str << "type: target flip, target offset: "
            << P->TargetOffsetInFragment.value()
            << ", Bit: " << P->Bit << (P->Sign? "+":"-");
      } else if(auto* P = std::get_if<TargetRange>(&Spec.Kind)) {
        Str << ", Range: " << P->TargetByteRange << "\n"
            << "  linked with Bundles " << P->NormalDest
            << " (normal dest) and " << P->FlipDest
            << " (flipped dest)";
      } else if(auto* P = std::get_if<TargetDestination>(&Spec.Kind)) {
        Str << "type: destination";
      } else if(auto* P = std::get_if<TargetNone>(&Spec.Kind)) {
        Str << "type: none";
  ...
  //This holds the assignment from a bundle to a victim address after
  //the "solver" determined a feasable layout
  struct Result {
    unsigned BundleIdx; //index in Bundles (MCAssembler::layout)
    std::optional<int> VictimFrame; //index in VictimInfos, if any
    std::optional<int> VictimFrameIdx; //index in element of VictimInfos, if any
    //the offset in de section at which the victim page starts
    std::optional<uint64_t> VictimPageOffset;
  };

  //The "solver" that determines a feasable layout (but does not actually
  //build the layout for the program under compilation)
  void solveFF(std::vector<Bundle>&, const MCAsmLayout&,
             std::map<uint64_t, Result>&);

  void printSectionLayout(MCAsmLayout&, MCSection&, std::vector<Bundle>&, bool);
...
public:
  ...
  //DBL add a boolean indicating whether it should create the custom layout
  //for the final binary (aka true for ROUND 2)
  void layout(MCAsmLayout &Layout, bool = false);
